...
public class TableUtils {
     ...
     * key: entityType.name
     */
    private static ConcurrentHashMap<String, HashMap<String, TableColumn>> entityColumnsMap = new ConcurrentHashMap<String, HashMap<String, TableColumn>>();

    /* package */
    static synchronized HashMap<String, TableColumn> getColumnMap(Class<?> entityType) {
    ...
    private static void addColumns2Map(Class<?> entityType, String primaryKeyFieldName, HashMap<String, TableColumn> columnMap) {
            ...
            for (Field field : fields) {
                ...
                if (ColumnConverterFactory.isSupportColumnConverter(field.getType())) {
                    if (!field.getName().equals(primaryKeyFieldName)) {
                        TableColumn column = new TableColumn(entityType, field);
                        if (!columnMap.containsKey(column.getColumnName())) {
                            columnMap.put(column.getColumnName(), column);
                        }
                    }
                } else if (ColumnUtils.isForeign(field)) {
                    TableForeign column = new TableForeign(entityType, field);
                    if (!columnMap.containsKey(column.getColumnName())) {
                        columnMap.put(column.getColumnName(), column);
                    }
                } else if (ColumnUtils.isFinder(field)) {
                    TableFinder column = new TableFinder(entityType, field);
                    if (!columnMap.containsKey(column.getColumnName())) {
                        columnMap.put(column.getColumnName(), column);
                    }
                }
    ...
    /* package */
    static TableColumn getColumnOrId(Class<?> entityType, String columnName) {
     ...
     * key: entityType.name
     */
    private static ConcurrentHashMap<String, TableId> entityIdMap = new ConcurrentHashMap<>();

    /* package */
    static synchronized TableId getId(Class<?> entityType) {
