package com.zhujohnle.mobidevos.framework.db;
...
import com.zhujohnle.mobidevos.framework.db.sqlite.SqlInfo;
...
public class DbTools {
   ...
   private SQLiteDatabase createDatabase(DaoConfig config) {
      ...
      if (!TextUtils.isEmpty(dbDir)) {
         ...
         Context mContext = config.getContext();
         File mFile = mContext.getApplicationContext().getDatabasePath(config.dbName);
         result = SQLiteDatabase.openDatabase(mFile.getPath(), null, SQLiteDatabase.OPEN_READWRITE);
      }
      return result;
   }
