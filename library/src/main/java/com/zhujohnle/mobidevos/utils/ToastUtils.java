package com.zhujohnle.mobidevos.utils;
...
public final class ToastUtils {
    ...
    public static void showShort(final CharSequence text, final int color,final int textColor) {
        show(text, Toast.LENGTH_SHORT, color,textColor);
    }

    private static void show(final CharSequence text, final int duration, final int color,final int textcolor) {
        HANDLER.post(new Runnable() {
            @SuppressLint("ShowToast")
            @Override
            public void run() {
                ...
                setBg(tvMessage, color);
                showToast();
            }
        });
    }

    private static void setBg(final TextView tvMsg, int color) {
        if (sBgResource != -1) {
            ...
            if (tvBg != null && msgBg != null) {
                tvBg.setColorFilter(new PorterDuffColorFilter(color, PorterDuff.Mode.SRC_IN));
                tvMsg.setBackgroundColor(Color.TRANSPARENT);
            } else if (tvBg != null) {
                tvBg.setColorFilter(new PorterDuffColorFilter(color, PorterDuff.Mode.SRC_IN));
            } else if (msgBg != null) {
                msgBg.setColorFilter(new PorterDuffColorFilter(color, PorterDuff.Mode.SRC_IN));
            } else {
